#!/bin/sh
# Self-contained Pangolin Manager — works when invoked via sh/dash (XPipe).
...
require_root() {
    if [ "$(id -u)" -ne 0 ]; then
        ...
        printf 'Example: sudo %s\n' "/tmp/xpipe/root/scripts/bash/panglin.sh" >&2
        exit 1
    fi
}
...
discover_services() {
    local unit file exec_line active enabled pid id
    SVC_NAMES=()
    SVC_ACTIVE=()
    SVC_ENABLED=()
    SVC_ID=()
...
draw_header() {
    ...
    printf '%s%b Pangolin Manager v%s%b' "$CYAN" "║" "$PANGOLIN_MANAGER_VERSION" "$RESET"
    pad=$((cols - 24 - ${#PANGOLIN_MANAGER_VERSION}))
    [ "$pad" -lt 0 ] && pad=0
...
draw_table_header() {
    ...
    draw_horizontal_rule "$(tput cols 2>/dev/null || echo 80)"
}

draw_service_rows() {
    local count="$1" idx name active enabled pid id state_color
...
register_new_service() {
    local pangolin_path raw_name unit p_id p_secret p_endpoint file tries instance
    ...
    while [ -z "$p_id" ]; do
        tries=$((tries + 1))
        [ "$tries" -gt 5 ] && enter_fullscreen && return 1
...
run_fallback_menu() {
    discover_services
    echo "Pangolin Manager v${PANGOLIN_MANAGER_VERSION} (non-interactive terminal)"
    if [ "${#SVC_NAMES[@]}" -eq 0 ]; then
        echo "No services found. Run on a real SSH terminal for the TUI."
        exit 0
    fi
    local idx
